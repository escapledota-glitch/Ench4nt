...
export function ProductDetailsReview({ productId, totalRatings, totalReviews, ratings = [], reviews = [] }) {
  const review = useBoolean();

  const total = sumBy(ratings, (star) => star.reviewCount) || 1;

  const renderSummary = () => (
    <Stack spacing={1} sx={{ alignItems: 'center', justifyContent: 'center' }}>
      <Typography variant="subtitle2">Дундаж үнэлгээ</Typography>
      ...
      <Rating readOnly value={totalRatings} precision={0.5} />

      <Typography variant="caption" sx={{ color: 'text.secondary' }}>
        ({fShortenNumber(totalReviews)} сэтгэгдэл)
      </Typography>
    </Stack>
  );
        ...
        .map((rating) => (
          <Box key={rating.name} sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            <LinearProgress
              color="inherit"
              variant="determinate"
              value={(rating.reviewCount / total) * 100}
              sx={{ mx: 2, flexGrow: 1 }}
            />

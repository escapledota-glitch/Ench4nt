...
import { useGetLocalProducts } from 'src/actions/product';
...
export function ProductListView() {
  ...
  const { products, productsLoading, mutate } = useGetLocalProducts();
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      // Handle both 'include' (normal select) and 'exclude' (select-all then uncheck) modes
      const idsToDelete = selectedRows.type === 'exclude'
        ? tableData.filter((row) => !selectedRows.ids.has(row.id)).map((row) => row.id)
        : [...selectedRows.ids];
      ...
      setTableData((prev) => prev.filter((row) => !deleteSet.has(row.id)));
      mutate();
      toast.success('Устгагдлаа!');
    } catch {
      toast.error('Устгахад алдаа гарлаа.');
    }
  }, [selectedRows, tableData, mutate]);
            ...
            slots={{
              ...
              toolbar: () => (
                <ProductTableToolbar
                  ...
                  selectedRowCount={selectedRows.type === 'exclude' ? Math.max(0, dataFiltered.length - selectedRows.ids.size) : selectedRows.ids.size}
